...
class DiscordNotificationPlugin(NotificationPlugin):
    ...
    def __init__(self, name: str = "discord", config: Optional[Dict[str, Any]] = None):
        ...
        webhook_url = self.config.get("webhook_url") or os.environ.get(
            "PING_DISCORD_WEBHOOK_URL"
        )

        if not webhook_url:
            raise ValueError(
                "Discord webhook URL 必須通過環境變數 PING_DISCORD_WEBHOOK_URL 或配置 webhook_url 設置"
            )
        ...
        **kwargs,
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                self.logger.error(
                    f"❌ Discord 通知發送失敗: HTTP {response.status_code}"
                )
                return False
