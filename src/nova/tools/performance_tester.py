...
class NovaPerformanceTester:
            ...
            stats.update(
                {
                    ...
                    "system_memory_available_gb": memory_info.available
                    / 1024
                    / 1024
                    / 1024,
    ...
    def time_function(self, func: Callable, *args, **kwargs) -> PerformanceResult:
        ...
        perf_result = PerformanceResult(
            ...
            network_sent_kb=end_stats["network_sent_kb"]
            - start_stats["network_sent_kb"],
            network_recv_kb=end_stats["network_recv_kb"]
            - start_stats["network_recv_kb"],
    ...
    def benchmark_function(
        ...
        stats = {
            ...
            "memory_usage": {
                ...
                "stdev": statistics.stdev(memory_usages)
    ...
    def calculate_performance_score(
        self, baseline_result: PerformanceResult, optimized_result: PerformanceResult
    ) -> Dict[str, float]:
        """計算效能改善評分"""
        # 計算改善倍數
        time_improvement = (
        ...
        memory_change = (
            optimized_result.memory_usage - baseline_result.memory_usage
        ) / (1024 * 1024)  # MB
        ...
        # I/O 效率評分
        io_improvement = (
            baseline_result.io_read_count + baseline_result.io_write_count
    ...
    def compare_performance(
        self, baseline_func: Callable, optimized_func: Callable, *args, **kwargs
    ) -> Dict[str, Any]:
        """比較兩個函數的效能差異"""
        print("🔍 效能比較測試")
        print("-" * 40)
        ...
        print(
            f"📊 基準函數 ({baseline_func.__name__}): {baseline_result.execution_time:.6f}s"
        )

        # 測試優化函數
        optimized_result = self.time_function(optimized_func, *args, **kwargs)
        print(
            f"⚡ 優化函數 ({optimized_func.__name__}): {optimized_result.execution_time:.6f}s"
        )
        ...
        with open(report_file, "w", encoding="utf-8") as f:
            ...
            if benchmark_results:
                ...
                for result in benchmark_results:
                    ...
                    f.write(
                        f"- **平均執行時間**: {result['execution_time']['mean']:.6f} 秒\n"
                    )
                    f.write(
                        f"- **執行時間標準差**: {result['execution_time']['stdev']:.6f} 秒\n"
                    )
                    f.write(
                        f"- **平均記憶體使用**: {result['memory_usage']['mean'] / (1024 * 1024):.2f} MB\n"
                    )
                    f.write(
                        f"- **平均 CPU 使用率**: {result['cpu_usage']['mean']:.2f}%\n"
                    )
                    f.write("\n")
            ...
            if self.results:
                f.write("## 單次測試結果\n\n")
                f.write(
                    "| 函數名稱 | 執行時間(s) | 記憶體使用(MB) | CPU使用率(%) | I/O讀取 | I/O寫入 | GPU使用率(%) |\n"
                )
                f.write(
                    "|---------|------------|---------------|---------------|---------|---------|---------------|\n"
                )
                ...
                for result in self.results:
                    ...
                    f.write(
                        f"| {result.function_name} | {result.execution_time:.6f} | {memory_mb:.2f} | {result.cpu_percent:.1f} | {result.io_read_count} | {result.io_write_count} | {result.gpu_usage_percent:.1f} |\n"
                    )
                ...
                if self.results:
                    ...
                    f.write(
                        f"- **系統記憶體使用率**: {latest.system_memory_percent:.1f}%\n"
                    )
                    f.write(
                        f"- **可用記憶體**: {latest.system_memory_available_gb:.2f} GB\n"
                    )
                    ...
                    if latest.gpu_count > 0:
                        ...
                        f.write(
                            f"- **GPU 記憶體使用**: {latest.gpu_memory_mb:.1f} MB\n"
                        )
    ...
    def decorator(optimized_func: Callable):
        ...
        def wrapper(*args, **kwargs):
            ...
            comparison = tester.compare_performance(
                baseline_func, optimized_func, *args, **kwargs
            )
